...
namespace fsu
{
  ...
  HashTableIterator<K,D,H> HashTable<K,D,H>::Insert (const K& k, const D& d)
  {
    ...
    {
      if ( (*j).key_ == k)            
        bucketVector_[i.bucketNum_].PopFront();       
      
      bucketVector_[i.bucketNum_].Insert(e);
    }
  ...
  {
    //increment the bucketItr
    //if bucketItr si not at the end of the bucket, return itr
    //if bucketItr is at end of bucket, start at beginning of next non-empty
    //bucket and return itr
    //if a non-emepty bucket is not found, return the end itr
