...
// unlink oldLink from list, return oldLink
{
  if(oldLink == 0)
  {
    std::cerr << "**No Link to Remove**\n";
    return 0;
  }

  Link * temp = oldLink->prev_;
  temp->next_ = oldLink->next_;
...
void List<T>::CheckLinks() const
{
  ...
  for (i = this->Begin(), n = 0; i != this->End(); ++i, ++n)
  {
    ...
    {
      std::cout << " ** forward reciprocity failure at position " << n << '\n';
      // ok = 0;
      break;
    }
  }
  // if (ok) std::cout << " ** passed forward reciprocity check\n";
  ...
  for (i = this->rBegin(), n = Size(); i != this->rEnd(); --i, --n)
  {
    ...
    if (!i.Valid() || !j.Valid() || !k.Valid() || i != j || i != k)
    {
      std::cout << " ** reverse reciprocity failure at position " << n << '\n';
      // ok = 0;
      break;
    }
  }     
  // if (ok) std::cout << " ** passed reverse reciprocity check\n";
} // CheckLinks()
