...
namespace fsu
{
  ...
  protected: // definitions and family
    ...
    struct Node
    {
      ...
      Node (const T& tval, uint8_t flags) : value_(tval), lchild_(0), rchild_(0), flags_(flags)
      {}
      ...
      static const char* ColorMap (uint8_t flags)
      {
        ...
        switch(flags)
        {
          case 0x00: return ANSI_COLOR_BOLD_BLUE;        // bits 00 = !RED |  ALIVE 
          case 0x01: return ANSI_COLOR_BOLD_BLUE_SHADED; // bits 01 = !RED | !ALIVE
          case 0x02: return ANSI_COLOR_BOLD_RED;         // bits 10 =  RED |  ALIVE
          case 0x03: return ANSI_COLOR_BOLD_RED_SHADED;  // bits 11 =  RED | !ALIVE
  ...
  template < typename T , class P >
  BST_BASE<T,P>::BST_BASE  () : root_(0), pred_()
  {}

  template < typename T , class P >
  BST_BASE<T,P>::BST_BASE  (P p) : root_(0), pred_(p)
  {}
  ...
  template < typename T , class P >
  BST_BASE<T,P>::BST_BASE  ( const BST_BASE& that ) : root_(0), pred_(that.pred_)
  {
  ...
  template < typename T , class P >
  typename BST_BASE<T,P>::InorderIterator
  BST_BASE<T,P>::BeginStructuralInorder() const
  {
    InorderIterator i;
    i.sInit(root_);
    return i;
  ...
  void BST_BASE<T,P>::Dump (std::ostream& os) const
  {
    ...
    current = Que.Front();
    Que.Pop();
    os << Node::ColorMap(current->flags_) << nodeFill << ANSI_COLOR_RESET;
  ...
  void BST_BASE<T,P>::Dump (std::ostream& os, int dw) const
  {
    ...
    while (!Que.Empty())
    {
      ...
      for (j = 0; j < currLayerSize; ++j)
      {
        ...
        os << Node::ColorMap(current->flags_) << std::setw(dw) << current->value_<< ANSI_COLOR_RESET;
        if (current->HasLeftChild())
        {
          Que.Push(current->lchild_);
          ++nextLayerSize;
