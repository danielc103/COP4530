...
namespace fsu
{
  ...
  void LevelorderBTIterator<C>::Increment()
  {
    if (!que_.Empty())
    {
      ...
      {              
        que_.Push(que_.Front()->lchild_);//that left child
      }
      if (que_.Front()->HasRightChild())
      {             
        que_.Push(que_.Front()->rchild_);//that right child
      }
      que_.Pop();
    }
    //return *this
  ...
  void ThreadedBTIterator<C>::Init(Node* n)
  {
    ...
    while(node_ && !node_->IsLeftThreaded())
    {           
      node_ = node_->lchild_;
    }
    while (node_ && node_->IsDead())
    {
      Increment();
    }   
    // requires implementation
  ...
  void ThreadedBTIterator<C>::rInit(Node* n)
  {
    ...
    while (node_ && !node_->IsRightThreaded())
    {      
      node_ = node_->rchild_;
    }
    while (node_ && node_->IsDead())
  ...
  void ThreadedBTIterator<C>::Increment ()
  {
    ...
    node_ = node_->rchild_;
    while ( node_ && !node_->IsLeftThreaded() && node_->lchild_)
      node_ = node_->lchild_;
     return;
    // requires implementation
  }
  ...
  void ThreadedBTIterator<C>::Decrement ()
  {
    ...
    node_ = node_->lchild_;
    while ( node_ && !node_->IsRightThreaded() && node_->rchild_)
      node_ = node_->rchild_;
     return;
    // requires implementation
  }
