/*
    ...
    oaa.h header file and implementation of Ordered Associative Array
    for graphs
*/
...
namespace fsu
{
  ...
  {
    PrintNode printnode(os,cw1,cw2);
    Traverse(printnode);
  }
  ...
  // recursive left-leaning insert
  {
    ...
    if (pred_(key,nptr->key_))
    {
      nptr->lchild_ = RInsert(nptr->lchild_, key, data);
    }
    else if (pred_(nptr->key_, key))
    {
